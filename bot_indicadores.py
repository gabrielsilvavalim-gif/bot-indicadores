...
class PDFRelatorio(FPDF):
    ...
    def header(self):
        ...
        self.fonte("", 10)
        self.cell(0, 6, self.safe(f"Indicador: {self.indicador} | Filial: {self.filial}"),
                  align="C", new_x="LMARGIN", new_y="NEXT")
        self.cell(0, 6, self.safe(f"Gerado em: {datetime.now().strftime('%d/%m/%Y %H:%M')}"),
                  align="C", new_x="LMARGIN", new_y="NEXT")
        self.ln(4)
...
def gerar_pdf(df, df_todas, indicador, filial, ano_selecionado):
    ...
    df_mom = calcular_mom(df, indicador)
